...
export function appAlert(Alert, sticky = false) {
	// location.hash = ""
	if (Alert !== null && Alert !== undefined && Alert.length !== 0) {
      ...
      <article class="pa0 absolute right-0 pt5 z-max" onclick={()=>appAlert()}>

					<div class="f6 ph2 pv1 near-white tl alertPrimary br2" style="animation-duration:5s">
						<pre class="avenir center">{alertReturn}</pre>
					</div>
    ...
    if (!sticky) {
      setTimeout(appAlertFadeOut, 20000);
    }
		// location.hash = "#top" ;
	} else {
