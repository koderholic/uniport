...
import {setTinyMCEContent} from './#pageTinyMce.js';
...
export function getPageSearchList(page) {
	...
	if(page.RefID !== undefined && page.RefID > 0) {
		pageSearchUrl += "&RefID="+page.RefID
	}
		...
		if (response.Code == 200) {
			if (response.Body !== null && response.Body !== undefined ){
				...
				if (document.getElementById("tinymce") != undefined){
					...
					console.log(document.getElementById("tinymce"));
					if (page.Form.File !== undefined) {
						if (tinymce.get("file") !== null) {
							switch(page.Mode) {
								case "edit":
									setTinyMCEContent("file", page.Form.File,false);
									break

								case "view":
									setTinyMCEContent("file", page.Form.File,true);
									break
							}
						}
					}

					if (page.Form.Description !== undefined) {
						if (tinymce.get("description") !== null) {
							switch(page.Mode) {
								case "edit":
									setTinyMCEContent("description", page.Form.Description,false);
									break

								case "view":
									setTinyMCEContent("description", page.Form.Description,true);
									break
							}
						}
					}
				...
				if (page.searchSubForm !== undefined && page.Form.ID !== undefined){
					page.searchSubForm();
				}

				if (page.updateToggle !== undefined && page.Form.ID !== undefined){
					page.updateToggle();
				}
			}
