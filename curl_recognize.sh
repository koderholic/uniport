...
echo "Recognizing $SourceFile with $Language language. Result will be saved in $OutFormat format.."

echo "Uploading.."
response=`curl -s -S --user "$ApplicationId:$Password" --form "upload=@$SourceFile" "$ServerUrl/processImage?exportFormat=$OutFormat&language=$Language&profile=TextExtraction&imageSource=Photo"`
